...
while True:
    try:
	current_info = master.recv_match().to_dict()
	flag1 = str_roll in current_info
	flag2 = str_pitch in current_info
	flag3 = str_yaw in current_info
	all_flag = flag1 * flag2 * flag3

	msg = str_roll + ':' + str(current_info[str_roll]) + ", "
	msg += str_pitch + ':' + str(current_info[str_pitch]) + ", "
	msg += str_yaw + ':' + str(current_info[str_yaw])
	if all_flag == True:
